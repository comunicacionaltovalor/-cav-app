...
// Componente para opciones únicas
...
export default function EvaluacionForm() {
  ...
  async function checkName() {
    ...
    setNameTaken((count ?? 0) > 0);
    setNameChecked(true);
  }

  function toggleQ2(topic: string) {
    setQ2(prev => prev.includes(topic)
  ...
  function validate(): string[] {
    const errs: string[] = [];
    if (!name.trim()) errs.push('Escribe tu nombre completo.');
    if (nameTaken) errs.push('Ya existe una respuesta con ese nombre.');
    if (!q1) errs.push('Q1: Selecciona una opción.');
    if (q2.length === 0) errs.push('Q2: Selecciona al menos un tema.');
    if (!q3) errs.push('Q3: Selecciona una opción.');
    Q5_ROWS.forEach(r => { if (!q5[r.key]) errs.push(`Q5: Califica "${r.label}".`); });
    if (!q6) errs.push('Q6: Selecciona una opción.');
    if (q8 === null) errs.push('Q8: Selecciona un número del 0 al 10.');
    if (!q9) errs.push('Q9: Selecciona una opción.');
    return errs;
  }
      ...
      <section className="shell" style={{ maxWidth: 720, margin: '0 auto' }}>
        ...
        <div className="card" style={{ marginTop: 24 }}>
          ...
          <div className="field">
            ...
            <input
              value={name}
              onChange={e => { setName(e.target.value); setNameChecked(false); setNameTaken(false); }}
              onBlur={checkName}
              placeholder="Ej: María González"
            />
            {nameTaken && (
              <p className="small danger" style={{ marginTop: 6 }}>
                Ya existe una respuesta con este nombre. Si cometiste un error al escribirlo, ajústalo ligeramente o contacta al instructor.
              </p>
            )}
          </div>
            ...
            <div style={{ marginTop: 8 }}>
              {Q2_TOPICS.map(topic => (
                <label key={topic} className="check">
                  <input
                    type="checkbox"
                    checked={q2.includes(topic)}
                    onChange={() => toggleQ2(topic)}
                  />
          ...
          <Q num="P4" text="¿Hubo un momento específico que cambió tu perspectiva sobre cómo comunicas? Descríbelo." optional>
            <textarea
              ...
              placeholder="(Opcional) Describe ese momento…"
              style={{ marginTop: 8 }}
            />
          </Q>
              ...
              <table style={{ minWidth: 480 }}>
                <thead>
                  <tr>
                    ...
                    {[1, 2, 3, 4, 5].map(v => (
                      <th key={v} style={{ textAlign: 'center', width: '11%' }}>{v}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                    ...
                    <tr key={row.key}>
                      ...
                      {[1, 2, 3, 4, 5].map(v => (
                        <td key={v} style={{ textAlign: 'center', padding: '12px 6px' }}>
                          <div
                            onClick={() => setQ5(prev => ({ ...prev, [row.key]: v }))}
                            style={{
          ...
          <Q num="P7" text="¿Qué podría hacer diferente el instructor para mejorar el programa?" optional>
            <textarea
              ...
              placeholder="(Opcional) Tu sugerencia…"
              style={{ marginTop: 8 }}
            />
          </Q>
            ...
            <div style={{ marginTop: 12 }}>
              ...
              <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
                {Array.from({ length: 11 }, (_, i) => {
                  ...
                  const bg = isSelected
                    ? (i >= 9 ? '#2E7D52' : i >= 7 ? 'var(--oro)' : '#7A1E1E')
                    : 'transparent';
                  const borderCol = isSelected
                    ? bg
                    : (i >= 9 ? 'rgba(46,125,82,.4)' : i >= 7 ? 'rgba(184,150,62,.5)' : 'rgba(122,30,30,.3)');
                  return (
                    <div
                      key={i}
                      onClick={() => setQ8(i)}
          ...
          <Q num="P10" text="¿Hay algo más que quieras compartir sobre tu experiencia?" optional>
            <textarea
              ...
              placeholder="(Opcional) Comentarios adicionales…"
              style={{ marginTop: 8 }}
            />
          </Q>
          ...
          <div style={{ borderTop: '1px solid var(--borde)', paddingTop: 24, marginTop: 8 }}>
            ...
            <div className="nav">
              <button onClick={submit} disabled={submitting || nameTaken}>
